...
public class ReflectUtils {
    ...
    public static String[] getArgTypes(Class[] parameterTypes) {
        String[] types = new String[parameterTypes.length];
        for (int i = 0; i < parameterTypes.length; i++) {
            types[i] = getDesc(parameterTypes[i]);
        }
        return types;
    }
     ...
     */
    public static Method findMethodBySignature(Class<?> clazz, String methodName, String[] parameterDescTypes)
            throws NoSuchMethodException, ClassNotFoundException {
        String signature = methodName;
        if (parameterDescTypes != null && parameterDescTypes.length > 0) {
            signature = methodName + StringUtils.join(parameterDescTypes);
        }
        Method method = Signature_METHODS_CACHE.get(signature);
        if (method != null) {
            return method;
        }
        if (parameterDescTypes == null) {
            ...
            Class<?>[] types = new Class<?>[parameterDescTypes.length];
            for (int i = 0; i < parameterDescTypes.length; i++) {
                types[i] = desc2Class(parameterDescTypes[i]);
            }
    ...
    private static Class<?> name2class(String name) throws ClassNotFoundException {
        int c = 0, index = name.indexOf('[');
        if (index > 0) {
            c = (name.length() - index) / 2;
            name = name.substring(0, index);
